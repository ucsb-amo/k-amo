...
class LightSheet(Beam):
    ...
    exactly one keyword from the size group.  ``u`` (the first waist) lies
    along ``transverse_axis``; ``v`` along ``k x u``.  The two axes have
    *different* Rayleigh ranges, ``zR_i = pi w_i^2 n / lambda``: the thin axis
    diverges fastest, and a single zR gets the axial confinement wrong by
    ``(w_u / w_v)^2``.

    Defaults (the lab's sheet, decided 2026-09-13): propagation along ``+z``,
    thin axis ``u`` along ``x`` so the sheet crosses the ``x`` tweezer and
    confines along it, wide axis along ``y``, linear polarization along ``y``.
    These differ from :class:`Tweezer`'s defaults (``+x``, circular), so a
    round ``LightSheet`` equals a ``Tweezer`` only when given the tweezer's
    frame and polarization explicitly.
    """

    def __init__(self, *, waist=None, NA=None, rayleigh_range=None,
                 divergence_angle=None, transverse_axis=(1.0, 0.0, 0.0),
